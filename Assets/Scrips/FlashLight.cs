...
public class FlashLight : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (mousePos.x > screenPos.x) {
            ...
            float degree = (Mathf.Atan((mousePos.y - screenPos.y)/(mousePos.x - screenPos.x))*180/Mathf.PI) - 90;

            // transform.Rotate(0f, 0f, mousePos.x*0.001f, Space.Self);
            target = Quaternion.Euler(0, 0, degree);
        } else {
            float degree = (Mathf.Atan((screenPos.y - mousePos.y)/(mousePos.x - screenPos.x))*180/Mathf.PI) - 90;
